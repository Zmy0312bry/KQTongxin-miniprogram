  ...
  <nut-row>
    <nut-col :span="24">
      ...
      <view v-else style="height: 200px">
        ...
        >
          <nut-swiper-item v-for="(item, index) in safeBannerList" :key="index">
            <img
              :src="item.imgUrl"
    ...
    <nut-row type="flex" justify="center" align="center" style="margin: 24px 0">
      <nut-button
        ...
        style="
          ...
          font-size: 16px;
          font-weight: bold;
          background: linear-gradient(135deg, #fa2c19 0%, #f56c6c 100%);
...
const safeBannerList = computed(() =>
  Array.isArray(bannerList.value) ? bannerList.value : [],
);
...
const fetchBannerList = async () => {
  try {
    const res = await Taro.request({
      url: `${API_BASE_URL}/community/banners`,
      method: "GET",
    });
    if (res.statusCode === 200 && res.data.code === 200) {
      ...
      bannerList.value = Array.isArray(list)
        ? list.map((item) => ({
            imgUrl: `${IMAGE_BASE_URL}${item.banner_image}`,
          }))
        : [];
    } else {
      bannerList.value = [];
...
const fetchWarmNotice = async () => {
  try {
    const res = await Taro.request({
      url: `${API_BASE_URL}/community/warm_notice`,
      method: "GET",
    });
    if (res.statusCode === 200 && res.data.code === 200) {
...
const fetchCarLimit = async () => {
  try {
    const res = await Taro.request({
      url: `${API_BASE_URL}/community/car_limit`,
      method: "GET",
    });
    if (res.statusCode === 200 && res.data.code === 200) {
